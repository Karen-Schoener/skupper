...
# Check if CA_TYPE is provided (root or intermediate)
# CA_TYPE=${1:-root}  # Default to "root" if not provided
CA_TYPE=${1:-intermediate}  # Default to "intermediate" if not provided
...
openssl x509 -req -in "$SKUPPER_LINK_DIR/link.csr" -CA "$CA_CERT" -CAkey "$CA_KEY" -CAcreateserial -out "$SKUPPER_LINK_DIR/tls.crt" -days "$DAYS" -extensions req_ext -extfile <(echo "$CERT_CONFIG") $CA_PASS 2>&1 | tee "$SKUPPER_LINK_DIR/tls_crt_debug.log"
